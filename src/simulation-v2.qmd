...
#' @param results List of estimates,each result is a vector, corresponding to estimators
#' @param mean The true mean in the setting
analyze_estimates = function(results, mean, caption = NULL) {
    # seq_along(results[[1]]) give a sequence of 1 to length(results[[1]])
    # .x gets the x-th element of the sequence
    # map_dbl(.x, .f) applies a function to each element of a list
    names.estimators = names(results[[1]])
    mean.estimators = map_dbl(seq_along(results[[1]]), ~ mean(map_dbl(results, .x)))
    bias.estimators = mean.estimators - mean
    var.estimators = map_dbl(seq_along(results[[1]]), ~ var(map_dbl(results, .x)))
    rmse.estimators = sqrt(var.estimators + bias.estimators^2)

    df.estimators = data.frame(
        Names=names.estimators,
        Bias=bias.estimators,
        Variance=var.estimators,
        RMSE=rmse.estimators
    )

    table = kable(df.estimators, caption)

    
    df.results =  map_dfr(results, ~ {
...
df_true.all$ImagePaths = paste0('<img src="./img/', df_true.all$Images, '.png" width="100" />')

df_true_transposed.all <- data.frame(t(df_true.all[c("Mean", "ImagePaths")]))
colnames(df_true_transposed.all) <- df_true.all$Images
rownames(df_true_transposed.all) <- c("Mean", "Histogram of Propensity Scores")

kable(df_true_transposed.all, escape = FALSE) %>%
  kable_styling()
```
...
estimates_11.linear.demystifing1 = pmclapply(1:n_rep, function(i) {
    c(
        unlist(obtain_results(params_11.linear[[i]]$demystifing1, Non_DR_Estimators.linear)),
        unlist(obtain_results(params_11.linear[[i]]$demystifing1, DR_Estimators.linear))
    )
